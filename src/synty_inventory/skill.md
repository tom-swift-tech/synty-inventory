...
- `part` — `class` (`body|cockpit|engine|wing|gear|greeble`), `mates_axis` (`-z` engine → rear socket of the body), `symmetric`, `size_class`.
...
5. Ship kits: one `body`, `cockpit` at +Z, engines at -Z, wings in mirrored ± X pairs, gear under -Y. Whole `SM_Ship_*` are references, not parts.
...
- `recipe ship_kit` must be `complete`; every part has `part.class` and measured `bounds`.
