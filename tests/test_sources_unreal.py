...
def test_discover_pack_dirs_normalizes_unmapped_slug(tmp_path: Path):
    (tmp_path / "polygon-city-01").mkdir()
    mapped = discover_pack_dirs(tmp_path)
    # No SLUG_PACK_OVERRIDES entries exist yet — falls back to normalization.
    assert mapped == {"Polygon_City": tmp_path / "polygon-city-01"}
...
def test_asset_index_exact_and_case_insensitive_match(tmp_path: Path):
    ...
    assert exact["SM_Bld_Apartment_01"] == "polygon-city-01/Content/Buildings/SM_Bld_Apartment_01.uasset"
...
def test_asset_index_unmapped_pack_returns_empty(tmp_path: Path):
    (tmp_path / "polygon-city-01" / "Content").mkdir(parents=True)
    exact, ci = asset_index(tmp_path, "POLYGON_City_Not_The_Guessed_Id")
    assert exact == {} and ci == {}
